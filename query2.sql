...
END;
GO
EXEC sp_PatientBillingStatement 21;
/*
...
BEGIN
    ...
    SELECT 
        ...
        Revenue,
        CASE 
            WHEN Revenue >= 2000000 THEN 'Yes'
            ELSE 'No'
        END AS TargetMet,
        Revenue - 2000000 AS SurplusOrDeficit
    FROM MonthlyRevenue
    ...
    SELECT
        'SUMMARY',
        NULL,
        'Met: ' + CAST(SUM(CASE WHEN Revenue >= 2000000 THEN 1 ELSE 0 END) AS VARCHAR(10))
        + ' | Not Met: ' +
        CAST(SUM(CASE WHEN Revenue < 2000000 THEN 1 ELSE 0 END) AS VARCHAR(10)),
        NULL
    FROM MonthlyRevenue;
END;
GO
...
BEGIN
    ...
    BEGIN
        ...
        IF EXISTS (
            ...
            HAVING SUM(b.FinalAmount) > 1000000
        )
        BEGIN
            ROLLBACK;
...
SELECT dbo.fn_CalculateNetBill(10000, 50000, 30000, 20) AS Bill;
...
-- Admin
GRANT CONTROL ON DATABASE::gautam_hospital TO db_admin;
